...
// City coords subset for geocoding (major cities)
const CITY_COORDS = {
...
const CURATED = [
  { id: 'web-summit-2026', title: 'Web Summit 2026', type: 'conference', location: 'Lisbon, Portugal', startDate: '2026-11-02', endDate: '2026-11-05', url: 'https://websummit.com', source: 'curated', description: 'World premier tech conference' },
  { id: 'collision-2026', title: 'Collision 2026', type: 'conference', location: 'Toronto, Canada', startDate: '2026-06-22', endDate: '2026-06-25', url: 'https://collisionconf.com', source: 'curated', description: 'North Americas fastest growing tech conference' },
  { id: 'gitex-global-2026', title: 'GITEX Global 2026', type: 'conference', location: 'Dubai, UAE', startDate: '2026-12-07', endDate: '2026-12-11', url: 'https://www.gitex.com', source: 'curated', description: 'Worlds largest tech and startup show' },
];
...
export const handler = async (event) => {
  ...
  try {
    if (path.endsWith('/list-tech-events')) {
      // Try Redis cache first (seeded by relay every 6h)
      const cached = await redisGet('research:tech-events:v1').catch(() => null);
      let events = [];

      if (cached?.events?.length) {
        events = cached.events;
      } else {
        // Try fetching Techmeme ICS directly
        try {
          const resp = await fetch('https://www.techmeme.com/newsy_events.ics', {
        ...
        } catch (e) { console.warn('[research] Techmeme fetch failed:', e.message); }
        events.push(...CURATED.filter(e => new Date(e.startDate) >= new Date()));
      }
      ...
      const days = parseInt(qs.days || '180', 10);
      const cutoff = new Date(); cutoff.setDate(cutoff.getDate() + days);
      events = events.filter(e => new Date(e.startDate) >= new Date() && new Date(e.startDate) <= cutoff);
      ...
      return ok({ success: true, count: events.length, conferenceCount: conferences.length, mappableCount: conferences.filter(e => e.coords && !e.coords?.virtual).length, lastUpdated: new Date().toISOString(), events, error: '' });
    }

    if (path.endsWith('/list-defense-patents')) {
      const data = await redisGet('research:defense-patents:v1');
      return ok(data || { patents: [] });
    }
