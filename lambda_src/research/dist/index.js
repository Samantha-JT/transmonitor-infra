...
var handler = async (event) => {
  ...
  try {
    if (path.endsWith("/list-tech-events")) {
      ...
      let events = cached?.events?.length ? cached.events : [];
      if (!events.length) {
        try {
          const resp = await fetch("https://www.techmeme.com/newsy_events.ics", { headers: { "User-Agent": "Mozilla/5.0" }, signal: AbortSignal.timeout(8e3) });
          if (resp.ok) {
          ...
          console.warn("[research] Techmeme:", e?.message);
        }
        const now2 = /* @__PURE__ */ new Date();
        events.push(...CURATED_EVENTS.filter((e) => new Date(e.startDate) >= now2));
      }
      events = events.map((e) => {
      ...
      return ok({ success: true, count: events.length, conferenceCount: conferences.length, mappableCount: conferences.filter((e) => e.coords && !e.coords?.virtual).length, lastUpdated: (/* @__PURE__ */ new Date()).toISOString(), events, error: "" });
    }
    ...
    return ok({ success: false, count: 0, conferenceCount: 0, mappableCount: 0, lastUpdated: (/* @__PURE__ */ new Date()).toISOString(), events: [], error: e?.message });
  }
};
