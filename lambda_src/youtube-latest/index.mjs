...
export const handler = async (event) => {
  ...
  if (!id && channel) {
    try {
      ...
      const r = await fetch("https://www.youtube.com/" + h, { headers: { "User-Agent": UA }, redirect: "follow", signal: AbortSignal.timeout(8000) });
      if (r.ok) {
  ...
  try {
    const fr = await fetch("https://www.youtube.com/feeds/videos.xml?channel_id=" + id, { headers: { "User-Agent": UA }, signal: AbortSignal.timeout(10000) });
