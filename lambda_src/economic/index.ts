...
const ALLOWED_FRED = new Set(['WALCL','FEDFUNDS','T10Y2Y','UNRATE','CPIAUCSL','DGS10','VIXCLS','GDP','M2SL','DCOILWTICO','BAMLH0A0HYM2','ICSA','MORTGAGE30US','GSCPI','T10Y3M','STLFSI4','DGS1MO','DGS3MO','DGS6MO','DGS1','DGS2','DGS5','DGS30','BAMLC0A0CM','SOFR','ESTR','EURIBOR3M','EURIBOR6M','EURIBOR1Y']);
...
export const handler = async (event: { rawPath?: string; path?: string; queryStringParameters?: Record<string, string>; requestContext?: { http?: { method?: string } }; body?: string }) => {
  ...
  try {
    if (path.endsWith('/get-macro-signals'))        return ok(await redisGet('economic:macro-signals:v1') ?? { verdict: 'UNKNOWN', bullishCount: 0, totalCount: 0, signals: {}, meta: {}, unavailable: true, timestamp: new Date().toISOString() });
    ...
    if (path.endsWith('/get-fred-series-batch')) {
      ...
      const ids = ((body.seriesIds || body.series_ids || []) as string[])
        .map(s => s.trim().toUpperCase()).filter(s => ALLOWED_FRED.has(s)).slice(0, 20);
      const settled = await Promise.allSettled(ids.map(id => redisGet(`fred:seed:${id}:v1`)));
      const results: Record<string, unknown> = {};
      for (let i = 0; i < ids.length; i++) {
        const r = settled[i];
        if (r.status === 'fulfilled' && (r.value as any)?.series) results[ids[i]!] = (r.value as any).series;
      }
