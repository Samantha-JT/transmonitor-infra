...
var handler = async () => {
  ...
  console.log(`feed-ingestor: running for variant=${variant}`);
  const digest = await buildDigest(variant);
  ...
  console.log(`feed-ingestor: wrote s3://${process.env.DIGEST_BUCKET}/${key}`);
  try {
    const redis = await getRedis();
    await redis.set(`digest:${variant}`, body, { EX: 1200 });
    console.log("feed-ingestor: Redis warmed");
  } catch (e) {
    console.warn("feed-ingestor: Redis write failed (non-fatal):", e.message);
  }
  return { statusCode: 200, body: "ok" };
};
async function buildDigest(variant) {
  return {
    variant,
    timestamp: (/* @__PURE__ */ new Date()).toISOString(),
    items: [],
    meta: { source: "feed-ingestor", version: "2.0.0" }
  };
}
export {
  handler
